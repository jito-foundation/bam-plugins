...
//   - authority address matches config.status_authority
fn checks(
    program_id: &Address,
    ...
    if authority.address() != &config_state.status_authority {
        return Err(ProgramError::MissingRequiredSignature);
    }
...
/// Accounts:
///   0. `[signer]`   authority (must match config.status_authority)
///   1. `[]`         config
...
pub fn process(program_id: &Address, accounts: &[AccountView], _data: &[u8]) -> ProgramResult {
...

    {
        ...
        if program_config_state.status != ProgramStatus::Enrolled {
            return Err(ProgramError::InvalidAccountData);
        }

        program_config_state.status = ProgramStatus::Active;
    }
